...
/**
 * @param seconds Number of seconds for which timer will run
 * @method starts showing a timer in count down manner for the given number of seconds
 */
fun TextView?.startTimer(seconds: Long, onFinishListener: () -> Unit): CountDownTimer? {
    val textView = this
    ...
    return object : CountDownTimer(millisInFuture, 1000) {
        ...
        override fun onTick(millis: Long) {
            val ms = String.format("%02d:%02d", TimeUnit.MILLISECONDS.toMinutes(millis) - TimeUnit.HOURS.toMinutes(TimeUnit.MILLISECONDS.toHours(millis)),
 ...
 * @method starts showing a timer in count down manner for the given number of seconds
 */
fun TextView?.startTimerHHMMSS(seconds: Long, onClickListener: () -> Unit) {
    val textView = this
    try {
        val millisInFuture = seconds * 1000
        object : CountDownTimer(millisInFuture, 1000) {
            ...
            override fun onTick(millis: Long) {
                val ms = String.format("%02d:%02d:%02d", TimeUnit.MILLISECONDS.toHours(millis),
...
fun TextView?.startTimerWithText(seconds: Long, mContext: Context, timerCompleted: (Unit) -> Unit) {
    val textView = this
    try {
        val millisInFuture = seconds * 1000
        object : CountDownTimer(millisInFuture, 1000) {
